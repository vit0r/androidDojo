...
public class CharacterListFragment extends ListFragmentUtil<Character> implements AdapterUtilInterface {
    ...
    public void requestAPI() {
        super.requestAPI();
        getMCompositeSubscription().add(getObservable().subscribe(getObserver()));
    }

    private Observer<SchemaData<Character>> getObserver() {
        return new Observer<SchemaData<Character>>() {
            @Override
    ...
    private Observable getObservable() {
        return getMMarvelServiceBase().getCharacters(Common.getParams(getResources(), getMList().size(), null))
